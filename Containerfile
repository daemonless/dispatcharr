...
#   psycopg[binary]: psycopg-binary has no FreeBSD wheels — drop the extra and
#     use pure psycopg (ctypes "python" libpq impl; libpq.so from postgresql17-client)
RUN sed -i '' \
        -e '/^    "torch/d' \
        -e '/^    "sentence-transformers/d' \
        -e '/^    "python-vlc/d' \
        -e 's/psycopg\[binary\]/psycopg/' \
        /app/pyproject.toml && \
